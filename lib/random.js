...
var RandomRoom = module.exports = function (room, io) {
	...
	var connectionHandle = function (socket) {
		...
		socket.on('chat with', function (partner, cb) {
			...
			if (ok) {
				var p = nicknames[partner];
				p.emit('private message', nickname, user.say, user.sex);
				socket.emit('private message', partner, p.user.say, p.user.sex);
			}
			cb(ok);
		});
		...
		socket.on('private message', function (msg, cb) {
			...
			if (typeof partner != 'undefined') {
				partner.emit('private message', nickname, msg, user.sex);
				cb(true);
			}
			else
				cb(false);
